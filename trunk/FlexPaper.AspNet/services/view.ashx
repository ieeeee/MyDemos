...
public class view : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        //doc参数实际为文档ID
        docID = context.Request["doc"].ToLower();
        page = context.Request["page"].ToLower();

        Guid g = Guid.NewGuid();
        if (Guid.TryParse(docID, out g))
        {
            //FlexPaper.AspNet.Codem.DBHelper dbh = new FlexPaper.AspNet.Codem.DBHelper();
            //FlexPaper.AspNet.Codem.DocModel m = dbh.getDoc(new Guid(docID));

            using (DocServiceClient client = new DocServiceClient())
            {
                Tencent.OA.DocEncyclic.Contract.Entity.Doc m = client.GetByDocID(new Guid(docID));
                ...
                else
                {
                    ...
                    context.Response.Write("doc 文档未找到.");
        ...
        {
            context.Response.Clear();
            context.Response.Write("doc 参数错误.");
            context.Response.End();
        }

        //获取Preview_URL（2015\01\57B1AF6029C072D2DA6BCBF609BF47D0\banche.pdf）
        //string Preview_URL = @"2015\01\380A3EF2B27FCF17AB7D9005D2B63FDE\banche.pdf";
        //string Preview_URL = @"2015\01\57B1AF6029C072D2DA6BCBF609BF47D0\CSharpLanguageSpecification.pdf";

        //根据Preview_URL：获取文档目录（2015\01\57B1AF6029C072D2DA6BCBF609BF47D0）
        string docDir = System.IO.Path.GetDirectoryName(Preview_URL).ToLower();

        //根据Preview_URL：获取文件名称（banche.pdf）
        doc = System.IO.Path.GetFileName(Preview_URL).ToLower();
        ...
        swfFilePath = Path.Combine(configManager.getConfig("path.swf"), docDir) + @"\" + swfdoc;

        pdfFilePath = configManager.getConfig("path.pdf") + Preview_URL;
        ...
        else
        {
            if (format == "swf" || format == "png" || format == "pdf" || format == "jpg")
            {
                ...
                //        {
                //            pdfconv = new pdf2swf(context.Server.MapPath(VirtualPathUtility.GetDirectory(context.Request.Path)) + @"..\");
                //            messages = pdfconv.convert(Preview_URL, page, context.Application);
                //        }
