...
namespace ClientApp
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            client.BaseAddress = new Uri("http://localhost:1345");

            HttpResponseMessage resp = client.GetAsync("api/ApiService").Result;
            resp.EnsureSuccessStatusCode();
            var rsStr = resp.Content.ReadAsAsync<String>().Result;

            Console.WriteLine("以下数据由Windows Service返回：");
